...
function HomePage() {
    ...
    async function getAllPost() {
        setBlogLoading(true);
        let posts = await fetch("https://one-profile-server.vercel.app/blog", {
            method: 'GET'
        });
    ...
    const handlePost = async () => {
        try {
            ...
            const response = await fetch('https://one-profile-server.vercel.app/blog', {
                method: 'POST',
                headers: {
    ...
    async function handleLike(blogId) {
        let token = localStorage.getItem('token');
        token = token.substr(1, token.length - 2);
        let finalCount = await fetch("https://one-profile-server.vercel.app/blog/like", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`
